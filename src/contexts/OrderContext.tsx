...
export const OrderProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const placeOrder = useCallback(
    (orderData: Omit<Order, "id" | "placedAt" | "estimatedMinutes" | "status">) => {
      const estimatedMinutes = 7 + Math.floor(Math.random() * 4);
      ...
      const preparingDelay = 2 * 60 * 1000;
      const outForDeliveryDelay = 5 * 60 * 1000;
      const deliveredDelay = estimatedMinutes * 60 * 1000;
